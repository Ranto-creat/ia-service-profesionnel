...
type Contours = { [key: string]: number[] }; // Dictionnaire associant une liste d'indices à chaque contour

export const drawFaces = (faces: Face[], ctx: CanvasRenderingContext2D, contours: Contours) => {
    for (let i = 0; i < faces.length; i++) {
        ...
        for (const [label, contour] of Object.entries(contours)) {
            ctx.strokeStyle = LABEL_TO_COLOR[label as keyof typeof LABEL_TO_COLOR] || "white";
